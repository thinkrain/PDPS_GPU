...
PairSPH_RHOSUMINCLUSION::PairSPH_RHOSUMINCLUSION(PDPS *ps) : Pair(ps)
{
	first = 1;
	newton_pair = 1;
	allocated = 0;
...
PairSPH_RHOSUMINCLUSION::~PairSPH_RHOSUMINCLUSION()
{
	if (allocated) {
...
void PairSPH_RHOSUMINCLUSION::allocate()
{
	allocated = 1;
...
void PairSPH_RHOSUMINCLUSION::compute(int eflag, int vflag)
{
...
void PairSPH_RHOSUMINCLUSION::set_style(int narg, char **arg)
{
	//	if (narg != 4)
...
void PairSPH_RHOSUMINCLUSION::set_coeff(int narg, char **arg)
{
	if (narg != 4)
...
void PairSPH_RHOSUMINCLUSION::init_one(int i, int j) {
...
double PairSPH_RHOSUMINCLUSION::single(int i, int j, int itype, int jtype,
...
int PairSPH_RHOSUMINCLUSION::pack_reverse_comm(int n, int first, double *buf) {
	int i, m, last;
	double *rho = particle->rho;
...
void PairSPH_RHOSUMINCLUSION::unpack_reverse_comm(int n, int *list, double *buf) {
	int i, m, j;
	double *rho = particle->rho;
	int *type = particle->type;
	int jtype;
...
int PairSPH_RHOSUMINCLUSION::pack_forward_comm(int n, int *list, double *buf) {
	int i, m, j;
	double *rho = particle->rho;
...
void PairSPH_RHOSUMINCLUSION::unpack_forward_comm(int n, int first, double *buf) {
	int i, m, last;
	double *rho = particle->rho;
