...
void Neighbor::half_multi(NeighList *neighlist)
{
	...
	for (i = 0; i < nlocal; i++) {
		...
		for (int io = 0; io < noffsets_multi[itype]; io++) {
			// find the cell index to be scan
			c1 = c + coffsets_multi[itype][io];

			// if c1 == c, the starting particle is linked_list[i] 
			// because the local id is increasing in the linked_list[i]
			if (c == c1) {
				j = linked_list[i];
			}
			else j = head[c1];
			...
			while (j != -1) {
				jtype = type[j];
				if (rneighsq[itype][jtype] < distsq_multi[itype][io]) {
					j = linked_list[j];
					continue;
				}
